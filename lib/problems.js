...
function pow(base, exponent) {
    ...
    if (exponent > 0) {
        return base * pow(base, exponent - 1) // 3^3     3 *  2^3
    } else {
...
function fileFinder(directories, targetFile) {
    for(let subDir in directories) {
        if (subDir === targetFile || fileFinder(directories[subDir], targetFile) === true) {
            return true;
        }
    }
    return false; 
...
function pathFinder(directories, targetFile) {
    let returnStr = "";
    for (let subDir in directories) {
        let result = pathFinder(directories[subDir], targetFile);
        if (subDir === targetFile) {
            returnStr += subDir;
        } else if (result) {
            let slash = "";
            if (result[0] !== "/") {
